...
void DissonanceMeeter::updateMeter(const juce::AudioBuffer<float>& buffer)
{
	// Calcola il livello RMS per ogni canale e aggiorna il meter
	for (int channel = 0; channel < buffer.getNumChannels(); ++channel)
	{
		auto* channelData = buffer.getReadPointer(channel);
		float rmsLevel = juce::Decibels::gainToDecibels(buffer.getRMSLevel(channel, 0, buffer.getNumSamples()));
		// Aggiorna il meter con il livello calcolato
		// Esempio: meter.setLevel(channel, rmsLevel);
	}
}
